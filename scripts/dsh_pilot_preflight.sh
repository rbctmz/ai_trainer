...
for line in sys.stdin:
    ...
    try:
        ...
        # печатается, но остальные записи остаются измеренными. При этом частичный
        # разбор — не измерение: счётчик уходит в метрики, и emit_metrics делает
        # прогон незачётным (иначе оплаченные токены и шаги были бы занижены
        # молча — см. находку ревью PR #584).
        parse_errors += 1
        continue
...
emit_metrics() {
  ...
  if [ -n "${SESSION_FILE:-}" ]; then
    ...
    if metrics_lines="$(zstd -dc "$SESSION_FILE" 2>/dev/null | python3 -c "$PARSER_PY")"; then
      ...
      elif printf '%s\n' "$metrics_lines" | grep -q '^session_parse_errors=[1-9]'; then
        # Частично нечитаемая сессия: валидные записи посчитаны, но итог занижен —
        # одна битая строка JSONL уменьшает токены, шаги и вызовы инструментов,
        # оставаясь при этом «успешным» измерением. Частичные значения выше
        # остаются напечатанными для диагностики, а зачёт прогона — нет.
        METRICS_FAIL=1
        echo "  ✘ сессия $SESSION_FILE прочитана частично (в ней есть нечитаемые записи): числа неполные и заниженные, прогон нельзя засчитывать по метрикам (для таймаутного прогона наружу всё равно 124)" >&2
      fi
    else
      METRICS_FAIL=1
