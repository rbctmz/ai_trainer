...
from api import onboarding_service, planning_profile
...
def get_planning_onboarding(db: Database = Depends(get_database)) -> Dict[str, Any]:
    event_context = onboarding_service.resolve_event_context()
    status = planning_profile.profile_status(db)
    return {
        **status,
        "suggested": onboarding_service.suggest_planning_defaults(
