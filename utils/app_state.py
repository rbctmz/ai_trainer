"""Typed-фасад состояния приложения без зависимости от Streamlit.

Issue #602. До этого разрыва продукт-API тянул legacy-UI цепочкой
``api/deps.py`` → ``from state import StateManager`` → ``state/manager.py`` →
``import streamlit``. Следствия были не только архитектурные: при импорте API
инициализировался runtime кэша Streamlit, а сам API не мог импортироваться без
установленного Streamlit.

Здесь живёт всё, что не требует Streamlit:

- обёртка над произвольным ``MutableMapping`` (в API это dict с доступом по
  атрибутам, в Streamlit — ``st.session_state``);
- дефолты состояния и ленивые провайдеры зависимостей (``database``,
  ``chat_manager``, ``ai_tools`` и прочие);
- typed-снимок ``snapshot()``.

Streamlit-специфика (``st.session_state`` по умолчанию и подсказка темы) живёт
в ``state.manager.StateManager``, который наследует этот фасад. Направление
зависимости — от Streamlit к фасаду, и никогда наоборот.

Модуль лежит в ``utils/``, а не в ``state/``: обращение к ``state.<имя>``
исполняет ``state/__init__.py``, который импортирует Streamlit-обёртку. Пакет
``state`` целиком является Streamlit-слоем, поэтому headless-код в нём жить не
может.
"""
...
class HeadlessState:
    """Typed-фасад состояния поверх произвольного mapping.

    Не импортирует Streamlit: фасад пригоден для FastAPI, тестов и любого
    другого headless-потребителя.
    """
    ...
    def __init__(self, session_state: MutableMapping[str, Any]) -> None:
        self._session: MutableMapping[str, Any] = cast(
            MutableMapping[str, Any], session_state
        )
    ...
    def _theme_base(self) -> Optional[str]:
        """Тема оформления по умолчанию — None в headless-режиме.

        Streamlit-обёртка переопределяет это, чтобы подхватить
        ``theme.base`` из ``.streamlit/config.toml``.
        """
        return None
...
class SessionDict(dict):
    """dict с доступом к элементам по атрибутам.

    ``HeadlessState.__setattr__`` делает ``setattr(self._session, key, value)``
    — для ``st.session_state`` это работает, а у обычного dict атрибутов нет,
    поэтому headless-запись (например сидирование демо-данных) падала бы.
    Этот класс замыкает ту же семантику на items.

    Переехало сюда из ``api/deps.py`` (issue #602): фасад состояния должен
    поставляться вместе со своим mapping, иначе каждый headless-потребитель
    изобретает свою копию.
    """
...
def make_headless_state(database: Any = None) -> HeadlessState:
    """Собрать headless-состояние поверх attribute-доступного mapping.

    Общая точка для всех headless-потребителей (API, сервисы, тесты): раньше
    каждый заводил свою копию. ``database`` биндится сразу, чтобы ленивое
    свойство ``database`` не создавало лишний ``Database()``.
    """
    session: SessionDict = SessionDict()
    if database is not None:
        session["database"] = database
    return HeadlessState(session)


__all__ = ["HeadlessState", "SessionDict", "make_headless_state"]
