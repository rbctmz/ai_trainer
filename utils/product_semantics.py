...
# Подписи шагов плановой тренировки для пользовательского текста. Ключ —
# `segment_kind` из `materialized_steps`, а НЕ `type` из проекции
# `plan_intervals`: у разминки, восстановления и заминки `type == "rest"` —
# это семантика матчинга план-факт, и по ней разминка отрендерилась бы как
# «отдых» (issue #638).
