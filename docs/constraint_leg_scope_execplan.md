...
**Итог (2026-08-19, ветка `fix/constraint-leg-scope`, PR #474 ready-to-merge).** Issue #473 закрыт по всем трём линиям: скоуп ограничения (M2), штатный откат дня (M3) и реальный ремонт инцидентa 18/08 в живой БД (M1). Вело-нога `ats_a163…` снова в плане 18/08 с подтверждённым матчем (факт 28.2 TSS) и `ready`-промптом; плавание зафиксировано как per-sport ограничение (#2), историческая whole-day строка #1 сохранена как append-only аудит.

### Что сработало
- **Один shared примитив** (`recover_day_after_constraint_retraction`) покрыл разовый M1 и будущие штатные откаты — ноль одноразового кода для инцидента, минимальная поверхность для регрессии.
- **Append-only дисциплина** (чекпоинты + матчи + constraints) дала полное lineage без перезаписи истории: repair-checkpoint #121 → donor #119, матч #24 supersedes-match на новый id. Аудит читаем «на глаз» из БД.
- **Identity-engine осознанность**: обнаружено правило #205 «одинокая нога наследует day-fingerprint» и выстроена явная handoff-карта + rebind — иначе вернувшаяся нога молча ушла бы в «вне плана» повторно.
- **BDD-first (RED→GREEN)**: каждый GREEN-срез закрывался тестом ещё на RED. Review #2 найденные находки (stale `duration_minutes`, отсутствие причины в `canceled_legs`-аудите) были реальными отклонениями контракта, пойманными ДО мержа — процесс работает как задумано.

### Уроки / что улучшить
- **Скоуп ограничений должен был быть с самого начала.** Корневая причина — LLM выбрал `date`, а механизм имел лишь whole-day семантику. Per-sport поле + правило выбора даты в промпте = двойной барьер. Обобщение: любое ограничение на день внутри многоногих дней триатлонного плана обязано нести скоуп с момента введения.
- **Инструменты восстановления нужны ДО инцидента.** Retract/repair существовали только как DELETE (деактивация без возврата) — пользователь не мог «отменить свою отмену». Теперь есть chat-tool + API; UI-кнопка отложена сознательно (Minimal Complexity).
- **Точное соответствие fixture реальным данным:** случайный `duration_minutes=40`=вело скрыл баг пересчёта до review #2. Фикстуры multi-leg дней должны иметь разные длительности ног, чтобы partial-перерасчёт ловился автоматически.
- **Бэкап через `sqlite_backup_restore.py --confirm-stopped`** подтвердил необходимость остановки стека для честного WAL-консистентного снимка перед любыми live-репарами.

### Остатки (осознанно вне этого PR)
- UI: кнопка «снять ограничение» + выбор sport в форме ограничения (см. Decision Log).
- Перенос `canceled_legs`-аудита в отдельную таблицу, если вырастет число потребителей (сейчас хватает template-level метаданных).
