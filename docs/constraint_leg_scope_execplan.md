...
- [ ] (pending) M3 GREEN: `recover_day_after_constraint_retraction` (donor walk по `checkpoint_parent_id`, `NoDonorCheckpointError`, `repair_evidence`), маршруты `POST /constraints/{id}/retract` + `POST /repair-day`, AI tool `retract_plan_constraint`.
