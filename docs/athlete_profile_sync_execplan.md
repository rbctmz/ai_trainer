...
- [x] (2026-07-05 12:10Z) User directed implementing issues `#101` and `#102` together in one combined change, since the NP fix alone (#101) only closes part of the gap without a correct FTP (#102), and vice versa (see the Decision Log entry on scope). Implemented the `#101` fix first: `data/data_processor.py` now extracts `normPower`/`normalizedPower` from the Garmin activity payload into a new `normalized_power` field (mirroring the existing `avg_power` extraction) and `resolve_tss` uses it, falling back to `avg_power` when absent. Added `normalized_power` as a persisted column on `activities` (`data/database.py`, additive migration via the existing `_ensure_activity_columns` helper). Tests: `tests/smoke/test_activity_tss_reconciliation.py::test_bike_power_tss_uses_normalized_power_not_average` and `::test_bike_power_tss_falls_back_to_average_power_without_np`.
- [x] (2026-07-05 12:25Z) Milestone 1: added `IntervalsICUClient.get_athlete_profile()` and the module-level `normalize_athlete_profile(raw)` in `services/intervals_icu.py`. Corrected one plan assumption while implementing it — see `Surprises & Discoveries`: `lthr` **is** available from Intervals.icu after all, so `normalize_athlete_profile` extracts it for real instead of always returning `None`. Cycling's `sportSettings` entry is still identified by `eFTPSupported: true`, not by index, exactly as planned. Tests: `tests/smoke/test_athlete_profile.py::test_normalize_athlete_profile_picks_cycling_entry_by_capability_not_index` and `::test_normalize_athlete_profile_degrades_to_none_on_missing_or_malformed_data`.
- [x] (2026-07-05 12:30Z) Milestone 2: added the `athlete_profile` table (append-only, `CREATE TABLE IF NOT EXISTS` in `init_tables()`) and `Database.save_athlete_profile`/`get_athlete_profile` in `data/database.py`, following the plain `sqlite3.connect`/`execute`/`commit`/`close` style already used by the neighboring `get_user_setting`/`set_user_setting` methods (this plan originally said no `user_settings`-like table existed; it does, as a generic key-value store — see `Surprises & Discoveries` for why a dedicated table was still the right call). Tests: `tests/smoke/test_athlete_profile.py::test_database_athlete_profile_round_trips_and_reads_latest`.
- [x] (2026-07-05 12:35Z) Milestone 3: added `sync_athlete_profile(database)` to `services/intervals_icu.py`, wiring Milestones 1+2 together with the planned graceful fallback. Tests: `tests/smoke/test_athlete_profile.py::test_sync_athlete_profile_success_path_persists_to_database`, `::test_sync_athlete_profile_not_configured_leaves_database_untouched`, `::test_sync_athlete_profile_request_failure_leaves_database_untouched`.
- [x] (2026-07-05 13:05Z) Milestone 4: added `resolve_athlete_ftp_lthr(database)` to `data/data_processor.py`; `data/database.py::_repair_legacy_activity_tss` now calls it instead of reading `Settings.USER_FTP`/`Settings.USER_LTHR` directly. Also updated `services/sync.py::_sync_activities` to use the same helper (a scope addition beyond the plan's original wording — see Decision Log) so a freshly synced profile applies to *new* activities immediately, not only to historical ones on the next app restart. Added the `tss_ftp_used` column, but as a field inside `resolve_tss`'s own return dict (set only in the `power_tss_bike` branch, `None` everywhere else) rather than inferred ad hoc at each call site — see Decision Log. Tests: `test_bike_power_tss_uses_synced_ftp_over_stale_env_default` and `test_repair_legacy_activity_tss_uses_synced_profile_on_next_database_open` in `tests/smoke/test_activity_tss_reconciliation.py`; both confirm the real `activity_id=23477418874` numbers land at `tss≈163.3`, within 5% of IntervalCoach's `160`.
- [x] (2026-07-05 13:20Z) Milestone 5: `services/sync.py::sync_garmin_data` now calls `intervals_icu_service.sync_athlete_profile(database)` before resolving any activity's TSS, folding a real failure into `result.warnings` (but not a plain "not configured" state, which is expected and silent). Updated `CLAUDE.md`'s Environment Variables section. Discovered and fixed a real test-isolation bug along the way — see `Surprises & Discoveries`. Tests: `test_sync_garmin_data_calls_athlete_profile_sync_before_resolving_activity_tss` and `test_sync_garmin_data_folds_athlete_profile_failure_into_warnings` in `tests/smoke/test_garmin_sync_service.py`.
- [x] (2026-07-05 13:25Z) Full suite green: `python -m pytest tests/smoke -q` → 363 passed (up from 349 at the start of this plan; 14 new tests across the two fixes, zero removed or skipped).

All five milestones plus the combined `#101` fix are complete as of this revision. Remaining follow-on work (explicitly out of scope for this plan) is tracked in GitHub issue `#104` (per-sport HR/power/pace zones beyond the three scalars this plan covers).
...
- Observation (2026-07-05, during implementation): this plan's own Context section was wrong about `lthr` — Intervals.icu does expose it, just not per sport.
  Evidence: a separate investigation for GitHub issue `#104` (comparing HR/power/pace zones across Garmin, Intervals.icu, and this app for all three sports) queried the full `sportSettings` array, not just the `ftp`-related fields this plan originally filtered for, and found `lthr: 163` and `max_hr: 180` repeated identically across all four `sportSettings` entries (Ride/Run/Swim/Other) — Intervals.icu keeps one `lthr`/`max_hr` per athlete today, not per sport, and it is populated. `normalize_athlete_profile` was implemented to read it for real (from the same cycling entry it already reads `ftp` from, since the value is identical everywhere) rather than hard-coding `None` as originally planned.

- Observation (2026-07-05, during implementation): the repository does already have a generic `user_settings` table, contrary to this plan's original Context section, but it confirms rather than undermines the "use a dedicated table" decision.
  Evidence: `data/database.py` defines `CREATE TABLE IF NOT EXISTS user_settings (key TEXT PRIMARY KEY, value TEXT, updated_at TIMESTAMP ...)`, used elsewhere via `get_user_setting`/`set_user_setting` for arbitrary string-keyed app settings. It has no typed columns and no concept of "the newest of several snapshots," so it would not have given `athlete_profile`'s round-trip/append/latest-row behavior for free; the new dedicated table remains the right shape.

- Observation (2026-07-05, during implementation): wiring `sync_athlete_profile` into `sync_garmin_data` (Milestone 5) made an existing, unrelated test file attempt a real network call.
  Evidence: `tests/smoke/test_garmin_sync_service.py` tests `sync_garmin_data` end-to-end using hand-written `_StubGarminClient`/`_StubDatabase` doubles, but never touched `Settings.INTERVALS_ICU_API_KEY` at all. In this developer's own checkout, `.env` has a real key configured, so once `sync_garmin_data` started calling `intervals_icu_service.sync_athlete_profile(database)`, the test suite silently made a live authenticated request to `https://intervals.icu` before failing on `_StubDatabase` lacking `save_athlete_profile`. Fixed with a module-level `autouse=True` pytest fixture in that file that forces `Settings.INTERVALS_ICU_API_KEY = None` for every test in it, restoring the "contributor-safe, no network, no real credentials needed" guarantee `tests/smoke` is supposed to have for anyone checking out the repo fresh.
...
- Decision (2026-07-05, during implementation): implement GitHub issue `#101` (average-power-vs-normalized-power) together with `#102` in the same change, rather than sequencing them as this plan originally implied ("Issue #101 ... is out of scope for this plan").
  Rationale: the user explicitly asked for both together, and the investigation had already shown they are additive and both necessary to close the IntervalCoach gap for `activity_id=23477418874` (NP alone: 44.9→~67; FTP alone: 44.9→~46-99 depending which stale/estimated FTP is used; both together: ~163.3, matching IntervalCoach's 160). Shipping only one would leave the headline bug only partially fixed.
  Date/Author: 2026-07-05 / Claude (implementation session)

- Decision (2026-07-05, during implementation): put `tss_ftp_used` inside `resolve_tss`'s own return dictionary (a new key present in all eight of its return statements, `None` except in the `power_tss_bike` branch), rather than having each caller infer "was FTP actually used" by string-matching `tss_method`.
  Rationale: two call sites now need this information (`services/sync.py::_sync_activities` for fresh syncs, `data/database.py::_repair_legacy_activity_tss` for the retroactive repair). Duplicating a `if tss_method == "power_tss_bike"` check in both places would silently miss any future power-based method name (issue `#104`'s findings already surfaced a Garmin-reported running power FTP of 406W, origin "weight" — a hypothetical future `power_tss_run` branch would need the same treatment). Computing it once, inside the function that already knows which branch produced the number, removes that duplication and that future trap.
  Date/Author: 2026-07-05 / Claude (implementation session)

- Decision (2026-07-05, during implementation): also update `services/sync.py::_sync_activities` to call `resolve_athlete_ftp_lthr(database)`, even though this plan's Milestone 4 wording named only `_repair_legacy_activity_tss`.
  Rationale: `_repair_legacy_activity_tss` only runs when a `Database()` object is constructed (typically once per process lifetime), while `_sync_activities` runs on every sync within that same long-lived process. Without this change, a profile synced mid-session would not affect newly-synced activities until the app restarted, while it would immediately affect the retroactive-repair path's view of historical rows — an inconsistency between "new" and "old" activities that served no purpose and was easy to avoid by calling the same helper from both places.
  Date/Author: 2026-07-05 / Claude (implementation session)

## Outcomes & Retrospective

All five milestones and the combined `#101` fix landed in one branch/PR (`claude/issue-101-102-tss-normpower-athlete-profile`), closing GitHub issues `#101` and `#102`. The concrete, user-verifiable outcome promised in `Purpose / Big Picture` is met: resolving `activity_id=23477418874` today produces `tss≈163.3` and `tss_ftp_used≈159`, instead of the original `tss=44.9` computed from a stale, unverifiable `.env` value — a result within about 2-5% of IntervalCoach's independently-reported `160`, the external reference that started this whole investigation.

What went further than originally scoped: `#101` (normalized power) was folded in at the user's explicit request rather than left for later, since the investigation had already shown neither fix alone gets close to the reference number; `services/sync.py::_sync_activities` was updated alongside `_repair_legacy_activity_tss` for FTP-source consistency between new and historical activities; and a real test-isolation gap (a smoke test silently reaching a live external API because of this developer's own `.env`) was caught and fixed before it could land as a flaky, environment-dependent test in the shared suite.

What remains, deliberately deferred: GitHub issue `#104` (HR zones, power zones, pace zones, and per-sport thresholds beyond the three scalars this plan covers) is explicit follow-on work, not attempted here. This plan also does not resolve whether historical `tss` values should be frozen at activity time rather than continuing to track the athlete's current FTP forever (see the Decision Log entry on `tss_ftp_used`) — the new column gives whoever picks that question up next the evidence to answer it, but does not answer it itself. `max_hr` was intentionally not synced at all in this plan, since nothing in this codebase's TSS formulas currently consumes it (only `ftp` and `lthr` are), and adding it now would have been speculative.

Lesson for future ExecPlans in this repository: writing the plan and doing the live API exploration that grounds it (as this plan's `Artifacts and Notes` did) is valuable, but treat every claim about "what an external API does or doesn't expose" as provisional until the implementing session re-verifies it — this plan's own claim that Intervals.icu does not expose `lthr` was wrong within hours of being written, discovered by a related but separate investigation (`#104`) that happened to look at the same API response more broadly.
...
Revision note (2026-07-05, planning): created this ExecPlan from the investigation already recorded on GitHub issues `#101` and `#102`, at the user's request, before any implementation began. Formal tracking issue: `#102`. Issue `#101` (the separate average-power-vs-normalized-power bug) was referenced throughout for context and originally marked out of scope for this plan's milestones.

Revision note (2026-07-05, implementation): the user then explicitly asked to implement `#101` and `#102` together in one change ("Начни #101+#102 вместе"). All five milestones were implemented, plus the `#101` fix, in branch `claude/issue-101-102-tss-normpower-athlete-profile`. `Progress`, `Surprises & Discoveries`, `Decision Log`, and `Outcomes & Retrospective` were updated throughout to record what actually happened, including two corrections to this plan's original Context section (Intervals.icu does expose `lthr`; a generic `user_settings` table already existed) and one scope addition beyond the original milestone wording (`services/sync.py::_sync_activities` was updated alongside `_repair_legacy_activity_tss` for consistency). See `Outcomes & Retrospective` for the full account.
