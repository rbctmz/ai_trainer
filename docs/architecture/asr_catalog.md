...
| ASR-REL-1 | Reconciliation: ни одна активность или исполнимая тренировка не теряется при перепланировании | High | content-derived session identity + lineage (`replaces_session_id`, #206/#209), append-only match/feedback/delivery ledgers; recovery/near-term мутации повторно материализуют точный catalog prescription; user unmatch retire-ит активный feedback всей superseded match-линии; explicit correction can reassign one complete inactive historical match without rewriting it (#531); reader сохраняет leaf/composite IDs и покрывает факт всех 16 отображаемых недель без расширения provider-I/O (#303); multisport read-проекция сохраняет envelope и все provider-linked этапы без двойного агрегата (#433); revisioned evidence-head CAS не даёт устаревшему Recovery Replan публиковать карточку или мутировать план (#552); modern broken sessions fail closed; recovery evidence identity включает версию правил: proposal штампуется `READINESS_CONFLICT_RULE_VERSION`, claim проверяет штамп независимо от evidence head, а атомарный sweep инвалидирует pending-карточки прежних правил на каждом прогоне (включая `data_gap`) (#557) | `test_recovery_transfer_identity_handoff.py`, twin-матрица identity, `test_recovery_replan_materialization_p1.py`, `test_issue_531_unplanned_activity_reassignment.py`, `test_planning_week_by_week.py`, `test_reconciliation_service_migration.py`, `test_api_planning.py`, `test_intervals_plan_delivery.py`, `test_api_activities_multisport.py`, `test_recovery_replan_loop.py` + `test_recovery_evidence_version.py` | ✅ |
| ASR-REL-2 | Отсутствие данных → data gap, не падение | High | gate-исходы silence/data_gap (#154), `has_plan=false` пробросы (#228/#301–#303); readiness salience учитывает persisted fatigue/recovery и legacy-safe fallback (#315); power-curve enrichment сохраняет кэш при provider failure; неполный multisport lineage оставляет envelope авторитетным и не теряет полученные этапы (#433); execution-role принимает только закрытый planner vocabulary; cognitive guardrails коуча требуют `данных недостаточно`/`сигналы смешанные` вместо выдуманной причинной истории (#465); directional drift использует только explicit event/checkpoint lineage, legacy и неполные связи остаются `data_gap` (#468); data gap не supersede-ит последнее полное recovery-решение (#552); интервенционный канал readiness отделён от описательного: пригодность требует подтверждённо сегодняшнего primary-измерения (сон/HRV/RHR), невалидные числа и измерения без provider-даты дают `data_gap`, provenance следует за значением и не наследуется новым значением (#557); датированное наблюдение `training_readiness` не откатывает метрику назад по дате измерения — writer сохраняет более свежие значение и дату и сообщает об отклонённой записи (#565); отсутствие или нечитаемость времени старта активности дают явный `activity_start_missing`/`activity_lookup_failed`, а непригодный снимок — `ineligible` с безопасной причиной, поэтому ложный pre-activity anchor не публикуется (#562) | smoke-гейты loop/ribbon + `test_readiness_conflicts.py`, `test_planning_active_plan_overview.py`, `test_planning_phase_roadmap.py`, `test_planning_week_by_week.py`, `test_best_efforts.py`, `test_actual_role_contract.py`, `test_api_activities_multisport.py`, `test_ai_coach_runtime.py`, `test_coach_drift.py`, `test_recovery_replan_loop.py` + `test_readiness_model.py` + `test_readiness_snapshot_contract.py` + `test_readiness_conflicts.py` + `test_sleep_metric_provenance.py` + `test_rhr_observation_provenance.py` + `test_hrv_training_readiness_provenance.py` + `test_issue_557_acceptance.py` + `test_recovery_capture_contract.py` + `test_recovery_capture_api_contract.py` | ✅ |
| ASR-REL-3 | Обрыв sync/maintenance не портит частичные данные | Medium | атомарный common-ingest; cursor-after-clean-batch; независимые activity/wellness cursors; SQLite restore через validated temp + atomic replace + pre-restore rollback; единая WAL/busy-timeout политика с race-гейтом (TD-003, #347); отказ производной post-sync capture не откатывает уже сохранённые данные провайдера: терминальный ответ становится `partial` со стабильным warning-кодом, а уже записанная ревизия сохраняет `snapshot_id`/`revision` (#562) | M0/M1 ingest/cursor, M4 wellness rollback, `test_sqlite_backup_restore.py` fail-before-replace и `test_sqlite_concurrency_policy.py` writer+reader + `test_recovery_capture_contract.py` + `test_garmin_sync_service.py` + `test_recovery_capture_intervals.py` + `test_issue_562_acceptance.py` | ✅ |
...
| ASR-MOD-2 | Новый компонент дашборда без регрессии | Medium | canonical snapshot проекции (#152/#153); `/planning` читает server-owned overview/week DTO без расчёта доменных метрик в TypeScript (#301–#303); `/activities` получает server-owned multisport group DTO вместо lineage-логики в React (#433); `/today` отдаёт server-owned `freshness`/даты факторов и intervention-поля, а страница только рендерит их (ярлыки, баннер, покрытие факторов) без пересчётов в TypeScript (#557); вердикт post-sync capture считает Python-домен и публикует очищенная проекция API, а `SyncControl` только отображает безопасные значения (человеко-читаемый статус и причина, локальное время или честный fallback, отсутствие идентификаторов и машинных кодов) без доменных расчётов в React (#562) | trust-alignment smoke + planning reader browser/API gates + `test_activities_multisport_ui_contract.py` + `test_readiness_today_freshness_ui_contract.py` + `test_recovery_capture_api_contract.py` + `test_m3_sync_ui_contract.py` | ✅ |
| ASR-MOD-3 | Смена схемы — обратная совместимость | Medium | аддитивные поля чекпойнтов, migrate-on-read (#206), append-only журналы; `intervals_plan_deliveries` добавлен через idempotent DDL и читается вместе с legacy proposal evidence; planning reader добавлен отдельными GET-контрактами без изменения checkpoint schema (#301–#303); coach decision/proposal lineage добавляется nullable migrate-on-start колонками (#468); recovery evidence ownership хранится в аддитивной head-таблице и proposal JSON, legacy rows остаются читаемыми и fail closed (#552); провенанс измерений добавлен аддитивными nullable-колонками (`sleep_score_observed_at`, `total_sleep_observed_at`, `rmssd_observed_at`, `resting_hr_observed_at`, `training_readiness_observed_at`) через migrate-on-start; legacy-строки читаются как `NULL`/`unverified` (#557); `recovery_capture` добавлен аддитивным полем терминального результата, провайдер — аддитивным `input_provenance.capture_provider` без изменения схемы и без миграции: прежние snapshots и episodes читаются без мутации (#562) | legacy-byte-equivalence + planning router/service contracts + `test_intervals_plan_delivery.py` + `test_coach_drift.py` + `test_recovery_replan_loop.py` + `test_sleep_metric_provenance.py` + `test_rhr_observation_provenance.py` + `test_hrv_training_readiness_provenance.py` + `test_recovery_capture_contract.py` + `test_recovery_capture_api_contract.py` | ✅ |
...
### Post-sync recovery snapshot capture parity (#562)

После синхронизации провайдера домен фиксирует производную запись — prospective
readiness-снимок — и публикует вердикт capture в терминальном результате job'а.
Контур замкнут на четырёх ASR (#562):

- **ASR-REL-3 — данные провайдера переживают отказ derived capture.** Capture —
  отдельная производная запись: если запись снимка падает, уже сохранённые
  активности/wellness не откатываются, ответ остаётся `partial` со стабильным
  warning-кодом, а вердикт несёт `capture_failed`. Если отказ случился после
  успешной записи ревизии (нечитаемая граница дня), сохранённые `snapshot_id` и
  `revision` остаются в ответе, а публичный текст исключения не раскрывается.
- **ASR-REL-2 — отсутствие данных даёт явный data gap, а не ложный anchor.**
  Отсутствие или нечитаемость времени старта активности дают
  `activity_start_missing` / `activity_lookup_failed`, непригодный снимок —
  `ineligible` с безопасной причиной (`low_confidence`, `missing_score`, …).
  Ложный pre-activity anchor не публикуется: вердикт считается по ревизии и по
  границе дня (минимальный старт активности дня, иначе локальный полдень), а не
  по «сегодня». Приоритет состояний явный: `capture_failed` →
  `activity_start_missing` → `ineligible` → `saved_before_load` / `saved_too_late`.
- **ASR-MOD-2 — статус и проекция принадлежат серверу.** Пять состояний, карта
  причин, вердикт и белый список публичных полей живут в Python
  (`models/recovery_response.py`, `services/recovery_analytics.py`); API отдаёт
  очищенный блок, а `SyncControl` только рендерит безопасные значения:
  человеко-читаемый статус и причину, локальное время атлета или честное «время
  не определено», без идентификаторов, машинных кодов и сырого `error`.
  Локализация кодов notice и снятие дублирования (notice, уже представленный
  структурным блоком) — тоже представление, без доменных расчётов в React.
- **ASR-MOD-3 — аддитивность без миграции.** `recovery_capture` добавлен
  аддитивным полем терминального результата, провайдер — аддитивным
  `input_provenance.capture_provider`; DDL и схема не менялись, прежние snapshots
  и episodes читаются без мутации, а `web/lib/types.ts` расширен опциональным
  полем с nullable-датами.

Идентичность рана — полный `capture_run_id` (UUID, рождается на границе job'а и
передаётся обязательным keyword-аргументом через provider-neutral runner);
короткий `job_id` остаётся display-хэндлом и штампуется менеджером job'ов.

Проверка: `test_recovery_capture_contract.py` (пять состояний, приоритет
предикатов, идемпотентность рана, чтение истории, провенанс),
`test_garmin_sync_service.py`, `test_recovery_capture_intervals.py` (паритет
Intervals), `test_recovery_capture_api_contract.py` (публичный набор полей и
отсутствие служебных данных), `test_issue_562_acceptance.py` (приёмка M6a с
запиненными JSON-фикстурами), `test_m3_sync_ui_contract.py` (статический
UI-контракт) и браузерная приёмка `tests/e2e/test_recovery_capture_readback.py`
(5 состояний × 2 провайдера через route interception).

Границы: это **не** live-provider E2E и не гарантия персистентности терминального
readback (менеджер job'ов process-local); вне объёма — backfill, polling-надстройки
и автоматическая коррекция плана. Полное разделение проверенного и непроверенного —
в evidence bundle [`../recovery_snapshot_capture_parity_evidence.md`](../recovery_snapshot_capture_parity_evidence.md).
