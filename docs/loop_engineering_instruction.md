...
events and explicitly check out the repository default branch. Direct review and
inline-review-comment events run only `.github/workflows/pr-review-signal.yml`,
which has `permissions: {}`, checks out no code, and exposes no secrets. Completion
of that signal wakes the trusted readiness workflow through `workflow_run`, so a
new review or thread comment invalidates stale acceptance promptly without giving
candidate-controlled YAML write authority. PR versions of the policy are tested
