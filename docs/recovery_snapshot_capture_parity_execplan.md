...
Ревизия документа: v1.16 (реализация: M1–M5, M6a и M6b закрыты, включая M6a-коррекцию `capture_failed` → `partial` и повторную браузерную приёмку; плюс hardening-слайс F1–F3; план отревьюен раундами `4ae3f4b`/`20f9dcb`, решения владельца D4/D5/D8 применены). Публичный API-контракт расширен аддитивно в M4, UI readback добавлен в M5; схема не менялась.
...
- Decision (D7): **расширять задачу за пределы issue нельзя**: без cron/фонового расписания, polling-надстроек, backfill исторических снимков, автокоррекции плана и provider writeback; правило pre-anchor не ослабляется, исторические `missing_pre_anchor` не переклассифицируются.
  ...
  Date/Author: 2026-09-12 / agent.
- Decision (M6b-re): **локализация кода в notices — зона UI, серверный текст предупреждения не меняется в этом слайсе**. `SyncControl::formatSyncNotice` переводит известные коды capture через `RECOVERY_CAPTURE_REASON_LABELS`; неизвестный код остаётся как есть (fail-open, без выдумывания причины).
  Rationale: инвариант «в UI нет машинных кодов» — принятое требование M5/M6b, и его нарушение после коррекции фикстуры было настоящим дефектом представления. Альтернатива — править формулировку warning'а в `services/sync.py`/`services/intervals_sync.py` — пересекает границу роли (Domain/API) и меняет продуктовый текст контракта, поэтому вынесена рекомендацией для M6c: либо локализовать предупреждение на сервере, либо не дублировать факт, который уже несёт `recovery_capture` (сейчас строка сбоя видна в UI дважды).
...
После коррекции M6a и повторной приёмки (см. ниже) строка для `capture_failed` начинается с «завершена частично» и несёт локализованное предупреждение; актуальный текст — в разделе `M6b re-acceptance`.
...
**Evidence для M6c:** каталог прогона `logs/e2e-recovery-capture/<stamp>/` (10 `.txt` с текстом строки и 10 `.png`), плюс два репрезентативных снимка, закоммиченных с `-f` (правило `*.png` в `.gitignore`): `docs/assets/issue_562_capture_readback_garmin.png` (счастливый путь с локальным временем) и `docs/assets/issue_562_capture_readback_intervals.png` (терминальный `partial` при отказе capture: честный fallback времени, причина и локализованное предупреждение). Оба снимка пересняты после коррекции фикстур (`20260912T214836Z`) — докоррекционные показывали зелёное «завершена».
...
**Наблюдаемый текст после исправления** (evidence `logs/e2e-recovery-capture/20260912T214836Z/`):
...
**Оба ассета пересняты** (не только Intervals): обе докоррекционные фикстуры несли `succeeded`, поэтому неверными были оба снимка, а не один. `docs/assets/issue_562_capture_readback_garmin.png` (111115 → 116165 B) и `docs/assets/issue_562_capture_readback_intervals.png` (111631 → 116045 B), оба 1280×900.

**Рекомендация для M6c (серверный текст предупреждения, вне зоны UI):** `services/sync.py` и `services/intervals_sync.py` формируют продуктовое предупреждение `⚠️ Recovery snapshot capture: <код>` — англоязычная обёртка плюс машинный код в пользовательской строке; UI теперь переводит код, но источник остаётся. Чище — либо отдавать локализованный текст с сервера, либо не дублировать факт, который уже несёт `recovery_capture` (в UI строка сбоя видна дважды: в warning и в readback-блоке). Это изменение домена/контракта, поэтому оно не делалось в UI-слайсе и вынесено решением ниже.
