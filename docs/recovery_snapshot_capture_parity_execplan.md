...
Ревизия документа: v1.17 (реализация: M1–M5, M6a и M6b закрыты, включая M6a-коррекцию `capture_failed` → `partial`, повторную браузерную приёмку и M6b-cleanup снятия дублирования; плюс hardening-слайс F1–F3; план отревьюен раундами `4ae3f4b`/`20f9dcb`, решения владельца D4/D5/D8 применены). Публичный API-контракт расширен аддитивно в M4, UI readback добавлен в M5; схема не менялась.
