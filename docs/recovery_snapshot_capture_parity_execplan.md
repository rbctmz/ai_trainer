...
**M5 — UI readback (роль UI / Design Specialist, D6).** `formatSyncJob` и рендер `SyncControl` показывают локальное время снимка, статус pre-activity и причину; строка не ломает существующие состояния (`running`/`failed`/`idle`), строка `notices` остаётся для `partial`. RED: статический UI-контрактный тест в `tests/smoke/test_m3_sync_ui_contract.py` (по образцу существующих) + проверка, что новые строки появляются для обоих источников.

**M6 — приёмка.** Синтетическая browser-приёмка обоих путей (D5), сквозные проверки идемпотентности/монотонности, fail-open, инварианта anchor, обновление `docs/architecture/asr_catalog.md` (ASR-REL-1/REL-2, ASR-MOD-2/3), evidence bundle в PR, запись метрик после мержа.
