...
Ревизия документа: v1.15 (реализация: M1–M5 и M6a закрыты, плюс hardening-слайс F1–F3; план отревьюен раундами `4ae3f4b`/`20f9dcb`, решения владельца D4/D5/D8 применены). Публичный API-контракт расширен аддитивно в M4, UI readback добавлен в M5; схема не менялась.
...
**Замечание для M6c (фикстурная верность):** фикстура `capture_failed` несёт `sync_state: "succeeded"`, тогда как в продакшене отказ capture добавляет warning и ответ становится `partial` (решение D4). Форму фикстур M6a менять нельзя по границам слайса, поэтому это зафиксировано здесь: сценарий проверяет readback блока, а не состояние синка; M6c может решить, обогащать ли фикстуру warning'ом.
