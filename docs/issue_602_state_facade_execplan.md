...
- [x] (2026-09-26) M0 / AC4 — набор снят пробником и запинен. Проба по двум
  dashboard-эндпоинтам через `TestClient` с записывающим фасадом даёт восемь
  имён: `ai_coach`, `database`, `goal_plan`, `latest_execution_feedback`,
  `latest_planning_checkpoint`, `planning_checkpoint_history`,
  `refresh_planning_checkpoint_cache`, `resolved_goal_plan_context`. По `grep`
  было видно только `database` — предупреждение issue о том, что ленивые
  свойства читают БД шире, подтвердилось. Пин проверен ломающей пробой: при
  удалении атрибута оба эндпоинта по-прежнему отвечают 200, а набор теряет
  имя и тест падает. Без пина регрессия прошла бы молча.
- [x] (2026-09-26) M5 — differential parity проведён по-настоящему.
  `scripts/api_parity_capture.py` снимает ответы всех 34 GET-маршрутов без
  path-параметров против детерминированной базы. Снятия на `main` (`b8c28d3`)
  и на ветке дали одинаковый sha256
  `0fa62ef7143db67f04927bd582e55c0fc951964e2c74a4c17d0e8f24849b7074` —
  payload-контракты и русские тексты идентичны.
- [x] (2026-09-26) M7 — PR #640 смержен в `main` как `d63df0c`
  (cycle time 4h 32m 54s). AC1–AC5 закрыты. Пост-мерж набор на `main`:
  2900 passed, 13 skipped, ruff чистый.
...
актуальный контрактный артефакт), а AC4 закрыт отдельным коммитом: набор из
восьми имён снят пробником и запинен, пин проверен ломающей пробой.

(2026-09-26) M5 закрыт. AC3 подтверждён сравнением, а не косвенно: снятия
ответов 34 GET-маршрутов на `main` и на ветке совпали байт в байт
(sha256 `0fa62ef7…`). Инструмент — `scripts/api_parity_capture.py`, он
переиспользуем и остаётся в репозитории, поэтому проверку можно повторить, а не
верить отчёту.

Что дало это сравнение помимо самого факта. Первое: наивное снятие показало
6 расхождений из 34, и все шесть оказались метками времени выполнения —
поведение не менялось, но без разбора это выглядело бы как регрессия. Второе:
заморозить часы подклассом `datetime.datetime` не получилось — процесс падает
SIGSEGV (pandas кэширует размер типа, exit 139), поэтому метки маскируются, а
имена полей по-прежнему сверяются. Третье: у `/api/planning/export/ics` UID
генерируется через `uuid4()`, то есть отличается всегда по устройству, и его
пришлось исключить из сравнения явно.
...
(2026-09-26) **M7 закрыт: PR #640 смержен в main как d63df0c**, cycle time
4h 32m 54s, issue lead time 8d 18h 24m 45s от создания issue #602. Пост-мерж
прогон на main после слияния обоих срезов: **2900 passed, 13 skipped**, ruff
чистый.

Что дал этот путь по существу. API перестал тянуть Streamlit на уровне
импорта: sys.modules после import api.main не содержит ни streamlit, ни
state, ни ui; grep по api/ не находит импортов legacy-пакета. Поведение
эндпоинтов подтверждено дифференциальным сравнением, а не только зелёным
набором: снятия 34 GET-маршрутов на main и на ветке совпали байт в байт.
Набор атрибутов состояния, который API реально читает, снят пробником —
восемь имён вместо одного, который подсказывал grep, — и запинен тестом,
чувствительность которого проверена ломающей пробой.

Цена и уроки. Два acceptance criteria (AC4 и M5) были закрыты уже после
перевода PR в ready for review: это процессный промах автора, зафиксированный
в docs/engineering_process_metrics.md вместе с остальными отклонениями этой
волны. Ветка прожила больше недели и устарела молча — её тесты выполняли AC2
отдельным файлом, тогда как issue просил расширить существующий контракт;
расхождение обнаружилось только при сведении с main. Отдельно: main защищён
ruleset-ом, а не classic branch protection, поэтому утверждение «required
checks нет» было неверным — обязательна ровно одна проверка,
Contributor-safe pytest, а Review gate в неё не входит.
...
`Outcomes & Retrospective`. Выполнено 2026-09-26.
...
Изменение 2026-09-26 (ревизия 3): M0/AC4 закрыт. Причина — критерий требовал
снять набор атрибутов состояния пробником и запинить его; сделано в
`tests/smoke/test_api_state_boundary.py` двумя тестами (пин и защита от
вырожденной пробы). Пин проверен ломающей пробой: удаление
`resolved_goal_plan_context` не ломает эндпоинты (они читают через
`getattr(..., None)`), но набор теряет имя и тест падает — без пина регрессия
прошла бы молча. Открытым остаётся только M5.

Изменение 2026-09-26 (ревизия 4): M5 закрыт. Причина — AC3 требовал
differential parity, и он проведён сравнением снятий на `main` и на ветке:
34 эндпоинта, совпадение байт в байт. Добавлен переиспользуемый инструмент
`scripts/api_parity_capture.py` (подкоманды capture и compare, код возврата 1
при расхождении). В `Surprises & Discoveries` дописаны три наблюдения:
6 расхождений от меток времени, SIGSEGV при попытке заморозить часы подклассом
datetime и uuid4-UID у ICS. Открытым остаётся только M7 — решение владельца о
мерже.

Изменение 2026-09-26 (ревизия 5): M7 закрыт. PR #640 смержен в main как
d63df0c, поэтому Progress и Outcomes & Retrospective приведены к финальному
состоянию, а указание «открытым остаётся только M7» в ноте ревизии 4 остаётся
историческим свидетельством о том ревизии, а не текущим статусом. Добавлено
наблюдение о ruleset: main защищён ruleset-ом, а не classic branch protection,
и обязательна ровно одна проверка — Contributor-safe pytest.
