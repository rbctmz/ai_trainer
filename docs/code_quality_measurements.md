...
- Числа зависят от окружения, и это часть baseline: без `web/node_modules` 16 тестов
  `test_contract_extractor.py` пропускаются, без локального `ai_trainer.db` — ещё 3,
  поэтому в чистом checkout (и в CI-джобе `contributor-safe-tests`, где npm-зависимости
  не ставятся) пропусков 38, а не 13. Сравнивать прогоны между собой можно только
...
Ожидаемый вывод: `Found 298 errors in 51 files (checked 145 source files)`.
...
3. Убрать его секцию `ignore_errors = True` из `mypy.ini`, если она есть;
   у модулей вне стартовых корней её нет — тогда шаг пропускается.
4. `python -m mypy` — убедиться, что проверка включает новый модуль (число
   проверенных файлов выросло) и разобрать ошибки как обычную правку типов (не
   менять поведение; при сомнении оставить `Any` и завести отдельный срез).
5. Обновить таблицы в этом документе: перенести модуль из долга в allowlist и
   пересчитать числа.
6. Когда `api/` и `services/` целиком окажутся в allowlist, ставить вопрос о
