...
M4a extends the existing `GET /api/planning/overview` reader contract without creating an endpoint or a mutation. `availability` shows the saved weekly available hours/minutes and days together with planned duration and leaf-session count calculated from the restored plan. `weekly_target_explanation` displays the persisted goal need, availability cap, recent load, base target, final target, and saved demand multiplier. A checkpoint without either source returns an explicit local data gap. Per-day available/planned comparison and overload/conflict messaging are intentionally absent: the current checkpoint does not save daily availability limits. Focused contracts cover saved context, legacy/missing fields, and the existing no-provider/no-mutation invariant.
