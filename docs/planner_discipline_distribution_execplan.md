...
- Decision: the weekly per-sport buckets keep being computed from `daily_plan` parts rather than being mechanically re-derived from sessions. Under strict same-discipline preservation, `parts == allocated_parts == sessions` holds by construction, and the three-way anchor enforces the equality of all three views on every build — a redundant mechanical re-derivation would add churn without adding truth. If a future fold ever records an explicit rebalance (sessions deviating from parts), the weekly derivation must switch to sessions at that moment; the anchor will fail loudly if this is forgotten. Date/Author: 2026-07-16 / Claude Code.
...
- Decision: keep four explicit layers with distinct mutability. `daily_plan` parts are the immutable per-discipline sport budget and the materialization input; M3 must not overwrite them before or after the split. `allocated_parts` (recorded on the day template) is the distributor's immutable decision of how the budget becomes sessions — equal to parts under same-discipline preservation. `sessions[]` materialize that decision. `weekly_summary` is a derived projection computed from the sessions. The anchor's budget reads the immutable parts, so it stays an independent check. Date/Author: 2026-07-15 / Greg.

- Decision: for the first M3, a sub-threshold discipline fragment becomes a short session of the same discipline on the same day rather than a cross-day merge. A cross-day merge would move load between days and so interacts with availability, recovery, and protected dates; that is out of scope for the first correct split and can be added later behind an explicit recorded rebalance. Date/Author: 2026-07-15 / Greg.
...
- [x] (2026-07-16) Milestone three (atomic): discipline split + per-session content ids + editor edit-by-`session_id` + Today web display of all sessions; three-way anchor GREEN (`budget == exported == table` per discipline, total conserved), legacy delivery identities unchanged (regression test), 696 smoke + 739 broad non-live + Next lint/build green.
    - [x] (2026-07-16) `_split_day_sessions`: one session per discipline carrying its exact budget share; feasible brick stays one composite session (`brick_status="grouped"`), infeasible brick falls back to independent same-discipline sessions with `brick_status="not_grouped"` plus the refusal reason — never a silent re-glue onto the dominant sport. `allocated_parts` recorded on the day; `daily_plan` parts never overwritten.
    - [x] (2026-07-16) Per-session identity: on multi-session days every id derives from session content with occurrence ordinals; single-session days inherit the day identity byte-for-byte (legacy compatibility).
    - [x] (2026-07-16) Editor: both rewrite sites purge stale `sessions`/`allocated_parts` and record `replaced_session_ids` (a day edit never drops a session silently); targeted `session_id` mode edits one session, preserves siblings, fails closed on unknown ids; edit rows expose the day's sessions.
    - [x] (2026-07-16) Legacy delivery regression: a pre-#205 plan yields byte-identical delivery events with and without the sessions wrapper; external ids remain prefix+day-id(+:leg:N).
    - [x] (2026-07-16) Today page renders every leaf session in order with brick-leg markers; `TodayLeafSession` type added.
...
- Observation: the first split run leaked exactly 72.8 TSS from run to bike while swim landed exactly on budget. Evidence: `exported={'bike': 2127.2, 'run': 1612.5, 'swim': 884.4}` against `budget={'bike': 2054.4, 'run': 1685.3, 'swim': 884.4}`. Cause: on days where the brick was infeasible, `materialize_brick_session` returned `legacy_role_fallback` and the helper collapsed the whole day onto the dominant sport. Fix: an infeasible brick falls through to independent same-discipline sessions and records `brick_status="not_grouped"` with the reason; the brick allocator is never allowed to change the sport budget.

- Observation: inheriting the day identity on multi-session days would let a sibling edit churn an untouched session's id. Evidence: the targeted-edit test failed with the untouched bike primary's id changing after a swim edit, because the day fingerprint includes whole-day total and parts. Fix: day-id inheritance applies only to single-session days (the legacy-equivalent shape, preserving all pre-#205 external ids); on multi-session days every session id derives from that session's own content.
