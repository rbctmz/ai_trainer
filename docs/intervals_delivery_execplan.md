...
- [x] (2026-08-02 18:23 MSK) Athlete-provided browser evidence closed the external observation: Intervals.icu rendered the 2, 5, and 7 August Run workouts with every step labelled `Pace`, and Garmin Connect showed the 2 August Recovery Run as an `Intervals.icu Training` calendar workout. This verifies Intervals target parsing plus downstream Garmin arrival; the Garmin screenshot does not independently expose step-level target rendering.
