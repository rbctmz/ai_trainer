...
**Status (2026-06-24): partial — gates pass, real-data load still blocked by provider throttle.** The pre-flight login probe returned `auth_ok=True` (the `mobile+cffi`/`mobile+requests` sub-strategies still 429'd, but `garminconnect`'s internal web/SSO fallback succeeded). However, the live UI probe run moments later only saw `429` again with **no** web fallback that time — so the welcome form reported "уже выполнено ✅" but no real activity/metric was loaded (`shows_real_activity_count: false`). Both probes used the identical code path (`garmin_connection.py:78` → `garmin_service.authenticate` → `GarminClient.authenticate`), so this is not an application bug: `garminconnect`'s web fallback is itself unstable under the same IP rate-limit and can succeed on one call and fail on the next. All page-render and AI-coach gates still passed cleanly (`0` exceptions, `0` alerts across dashboard / plan / activities / HRV / sleep / coach + AI message), and the welcome banner now reads "реальный Garmin login разрешён" honestly (P1a) with alerts now captured by the probe (P1b). Conclusion: the acceptance harness is healthy and observably honest; a clean real-data end-to-end run still needs the Garmin IP throttle to fully clear (wait several more hours, not minutes) before retrying the full-run commands below.
