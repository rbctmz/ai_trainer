...
Проверка лексики должна быть узкой. Репозиторный поиск по `safe` даёт ложные срабатывания на `_safe_float`, `safe_deliver_active_plan`, `safeRecoveryVariants` и `web/package-lock.json`, поэтому он не может служить критерием. Ищите риск-слова только в пользовательских подписях ACWR, например `grep -rn "injury\|риск травмы" models/acwr.py web/components`, а надёжнее — утверждать отрендеренные подписи полос напрямую в тесте.
