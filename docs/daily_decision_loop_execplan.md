...
чистой копии репозитория переключитесь на эту ветку, затем запустите
`ai_trainer_env/bin/python -m pytest tests/smoke/test_dashboard_metrics_anchor.py -q`
...
**#608 — безопасная семантика нагрузки.** Поиск по пользовательским строкам
(`grep -rn "safe\|optimal\|moderate_risk\|high_risk" models/acwr.py api/ web/`)
не должен находить риск-лексику. Повышенное отношение **без** свежей
готовности, симптомов, обратной связи и фактов сессии обязано дать описательный
контекст и не предписывать снижение или отдых; тот же вход **со** свежими явными
симптомами обязан дать предписание, ссылающееся на симптомы, а не на ACWR.
Численные фикстуры остаются неизменными: `python -m pytest tests/smoke/test_acwr.py -q`.
...
`python -m pytest tests/smoke/test_plan_actual_reconciliation.py -q` плюс focused
наборы activity/feedback/Today.
...
- **Observed**: прогон `tests/test_full_sync_cycle.py` против копии реальной
  базы (`VACUUM INTO`) переписал строку `2026-09-19`: `424 / 48 / 262 / 114 /
  77.0` стали `395 / 12 / 357 / 26 / 57.0`, а провенанс `garmin` понизился до
  `legacy_unknown`. Поле `created_at` при этом не изменилось, поэтому по нему
  потерю не заметить.
...
Отсюда правило: либо запускайте набор с фильтром маркеров, как в примерах выше,
либо изолируйте базу явно — `DATABASE_PATH=<временный путь> pytest ...`.
Никогда не направляйте эти команды на рабочую `ai_trainer.db`.
