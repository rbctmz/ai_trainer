...
- Decision: источником фактической структуры для матчинга являются круги Garmin, когда они сохранены рядом с интервалами провайдера; автодетект Intervals.icu (`icu_intervals`) — fallback (#460).
  Rationale: автодетект провайдера строит интервалы по смене зоны мощности (границы `icu_power_zones` в %FTP) и сливает мягкие тренировки, чьи шаги не пересекают зоны, в один блок на всю сессию (кейс 2026-08-17: шаги 94/117/129/69 Вт при FTP 172 — все внутри Z2 → «1 участок 40:05» → «+82%, Факт: нет» ×3 при выполненном по кругам плане 4/4). Круги устройства — первоисточник выполненной структуры; follow-up #462 — нарезка по стримам для пользователей без Garmin.
  Date/Author: 2026-08-18 / ZCode.

- Decision: флаг «возможен рассинхрон с устройством» гасится, когда текущий чекпоинт был успешно доставлен повторно до старта активности (#461).
  Rationale: правило #398 видело только «доставку более ранней версии» и не учитывало успешную редоставку — кейс 2026-08-17: реплан 05:48, доставка checkpoint 119 `success` 05:48–05:52, старт тренировки 16:54 (через 11 часов), а предупреждение всё равно показывалось. Подтверждение редоставки — только явный `status=success` для этой даты с `created_at <= started_at_utc`; иначе флаг остаётся (консервативно).
  Date/Author: 2026-08-18 / ZCode.
...
После визуальной приёмки усилены разделители: каждый следующий сгруппированный этап получает явную начальную линию наряду с внутренними линиями Auto Lap. Поэтому шесть исходных участков всегда дают пять видимых границ, даже если соседние этапы имеют одинаковый зелёный цвет и близкую высоту.
