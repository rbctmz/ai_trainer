...
`_repair_legacy_activity_tss()` выполняется при инициализации БД (старте
приложения) и пересчитывает строки, чей сохранённый TSS не соответствует
актуальным правилам; синк пересчитывает только активности в окне
синхронизации. Для вело-строк с мощностью repair использует FTP, актуальный
**на дату тренировки** (append-only история `athlete_profile`), поэтому смена
FTP не переписывает прошлые TSS молча. После изменения профильного порога без
датированной истории (например, нового CSS) история обновится при следующем
старте.
...
При пересчёте истории (`_repair_legacy_activity_tss`) для мощности
используется FTP на дату активности из истории снэпшотов `athlete_profile`
(`ftp_at` в `data/data_processor.py`), а не последний профиль; синк
по-прежнему берёт последний профиль.
...
В расчёт значение передаётся через `resolve_athlete_tss_profile()`
(`data/data_processor.py`), который возвращает `(ftp, lthr,
swim_threshold_pace_seconds_per_100m)`; `resolve_athlete_ftp_lthr()` остаётся
обратно-совместимой обёрткой на `(ftp, lthr)`.
...
- **Swim Pace TSS (sTSS)**: `duration_hours × IF³ × 100`, где
  ...
  Пример (тренировка 2026-08-03, 1.6 км за 40.9 мин движ. = 153.3 с/100м, CSS
  2:18/100м = 138 с/100м): `IF = 138/153.3 = 0.900`, `0.6814 ч × 0.900³ × 100 =
  49.7 TSS` — вместо 23.0 по прежней ЧСС-ветке.
