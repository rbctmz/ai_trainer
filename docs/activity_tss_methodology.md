...
приложения) и пересчитывает **все** строки; синк пересчитывает только
активности в окне синхронизации. Поэтому после изменения профильного порога
(например, нового CSS) история полностью обновится при следующем старте.
