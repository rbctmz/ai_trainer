...
Ревизия документа: v3.11 (M1–M3 закрыты и приняты; M4 — fail-closed gate и version-qualified ownership — выполнен, см. `Change log`).
...
- Decision (M4): интервенционный гейт читает **только** интервенционный канал: `intervention_score` вместо `score` и `intervention_confidence` вместо `confidence`; отчёт получает `rule_version` (`READINESS_CONFLICT_RULE_VERSION = "readiness_conflicts_v2"`) и блок `readiness` с intervention- и legacy-значениями (`legacy_score`/`legacy_confidence` сохранены для аудита, но порог не поднимают). Fallback на legacy-поля намеренно отсутствует: фикстуры и вызовы, которые ещё передают только `score`, обновляются, а не получают старое поведение.
