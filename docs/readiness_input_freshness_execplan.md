...
- v3.17 (2026-09-11): post-merge запись по PR #563 (docs-only, Class C).
  Зафиксирован merge в `main` коммитом `a82be7b` и автоматическое закрытие issue #557; указаны два нативных раунда review (полный на `a04aca3` и scoped delta на `4b2db94`), классификации `fixed-in 73645be` и follow-up #564/#565, состояние Review gate (ждёт acceptance владельца, не код) и зелёный CI на merge-коммите. Описание AC10 приведено к фактической реализации — реальный HTTP `GET /api/today` через `TestClient(app)` с `dependency_overrides[get_database]`, а не прямой вызов `today_view`. Число изменённых файлов в разделе `Plan of Work` убрано, чтобы docs-правки его не рассинхронизировали. Пост-мержевые метрики процесса — `docs/engineering_process_metrics.md`.
