...
Ревизия документа: v3.4 (после приёмки M1 и M2; M3, срез 3.1 переработан по ревью, см. `Change log`).
...
Дифференциальный пробник `legacy_parity_probe.py` против `origin/main` `9a46087` — **идентично** (повторён после переработки). `ruff check` по изменённым файлам чист.
