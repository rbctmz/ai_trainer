...
Ревизия документа: v3.18 (M1–M6 закрыты; follow-up #565 — guard против отката `training_readiness` по дате измерения — в работе, см. `Follow-up #565`; PR #563 смержен в `main` merge-коммитом `a82be7b` после двух раундов нативного review — 5 находок `fixed-in 73645be`, 2 новые P2 вынесены в follow-up [#564](https://github.com/rbctmz/ai_trainer/issues/564)/[#565](https://github.com/rbctmz/ai_trainer/issues/565); см. `Change log` и `Artifacts and Notes`).
...
- [ ] (2026-09-11) Follow-up #565: `sync_training_status` не должен откатывать `training_readiness` на более старую дату измерения. Класс — A (автоматические триггеры: владение провенансой и семантика persistent-записи), контур right-sized: этот ExecPlan без нового документа, slice-spec `docs/issue_565_readiness_write_guard_slice_spec.md`, один RED→GREEN-слайс. См. `Follow-up #565` в `Artifacts and Notes`.
...
- Decision (follow-up #565): инвариант «дата измерения `training_readiness` монотонна в пределах строки дня синка» обеспечивается **внутри write-заявления**, а не предварительным чтением.
  Rationale: `sync_training_status` читает сохранённую провенансу в начале вызова, поэтому Python-guard оставлял бы окно TOCTOU между `SELECT` и `UPDATE` — в этом репозитории на одну SQLite-базу смотрят API, legacy-Streamlit и тесты, то есть два писателя теоретически пересекаются. Для readiness-колонок `UPDATE` теперь содержит `CASE WHEN date(?) IS NOT NULL AND date(training_readiness_observed_at) IS NOT NULL AND date(training_readiness_observed_at) > date(?) THEN training_readiness ELSE ? END` (симметрично для даты), поэтому решение принимает текущая строка. Python-решение из предварительного чтения управляет только счётчиком `stale_readiness_rejected` и warning'ом синк-слоя.
