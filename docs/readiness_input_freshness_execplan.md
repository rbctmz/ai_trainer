...
Ревизия документа: v3.20 (M1–M6 закрыты; follow-up #564 — интервенционное evidence в тексте конфликта — в работе, см. `Follow-up #564`; follow-up #565 закрыт — PR #567 смержен в `main` коммитом `63773ed` после одного чистого нативного раунда, см. `Follow-up #565`; PR #563 смержен в `main` merge-коммитом `a82be7b` после двух раундов нативного review — 5 находок `fixed-in 73645be`, 2 новые P2 вынесены в follow-up [#564](https://github.com/rbctmz/ai_trainer/issues/564)/[#565](https://github.com/rbctmz/ai_trainer/issues/565); см. `Change log` и `Artifacts and Notes`).
...
- [ ] (2026-09-12) Follow-up #564: текст evidence конфликта/предложения должен описывать ту же серию, по которой считался `intervention_score_input`. Класс — A (аддитивный cross-module public contract + identity evidence: текст входит в `_fingerprint`), контур right-sized: slice-spec `docs/issue_564_intervention_evidence_slice_spec.md`, один RED→GREEN-слайс, регенерация `ts_contract.json` и web-проверки. См. `Follow-up #564` в `Artifacts and Notes`.
