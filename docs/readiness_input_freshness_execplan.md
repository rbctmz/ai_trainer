...
Ревизия документа: v3.22 (M1–M6 закрыты; follow-up #564 закрыт — PR #569 смержен в `main` коммитом `43984fa` после одного чистого нативного раунда, см. `Follow-up #564`; follow-up #565 закрыт — PR #567 смержен в `main` коммитом `63773ed` после одного чистого нативного раунда, см. `Follow-up #565`; PR #563 смержен в `main` merge-коммитом `a82be7b` после двух раундов нативного review — 5 находок `fixed-in 73645be`, 2 новые P2 вынесены в follow-up [#564](https://github.com/rbctmz/ai_trainer/issues/564)/[#565](https://github.com/rbctmz/ai_trainer/issues/565); см. `Change log` и `Artifacts and Notes`).
...
- [x] (2026-09-12) Follow-up #564 закрыт: текст evidence конфликта/предложения описывает ту же серию, по которой считался `intervention_score_input`. Класс — A (аддитивный cross-module public contract + identity evidence: текст входит в `_fingerprint`), контур right-sized: slice-spec `docs/issue_564_intervention_evidence_slice_spec.md`, один RED→GREEN-слайс, регенерация `ts_contract.json` и web-проверки. См. `Follow-up #564` в `Artifacts and Notes`.
...
Merge: PR [#569](https://github.com/rbctmz/ai_trainer/pull/569) смержен в `main` коммитом `43984fa` (8 файлов, +461/−10); `git diff 565a6d3 43984fa` пуст, issue #564 закрыт автоматически, нативный раунд на текущем head — один и чистый (`review-gate/codex-clean/5644835586`), бюджет 1/2. CI на merge-коммите зелёный; числа прогонов в записи метрик разведены: CI `2472 passed, 27 skipped, 26 deselected`, локальный worktree с `node_modules` — `2494 passed, 5 skipped`.
...
- v3.22 (2026-09-12): post-merge запись по follow-up #564.
  PR #569 смержен в `main` коммитом `43984fa` (8 файлов, +461/−10), issue #564 закрыт автоматически, дерево в main идентично отревьюенному head `565a6d3`. Нативный раунд — один, чистый, без находок (бюджет 1/2); CI на merge-коммите зелёный. Метрики Class A по этому изменению — в `docs/engineering_process_metrics.md`, где CI- и локальный прогоны подписаны раздельно (2472/27/26 против 2494/5).
