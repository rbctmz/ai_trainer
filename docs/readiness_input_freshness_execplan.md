...
Ревизия документа: v3.19 (M1–M6 закрыты; follow-up #565 закрыт — PR #567 смержен в `main` коммитом `63773ed` после одного чистого нативного раунда, см. `Follow-up #565`; PR #563 смержен в `main` merge-коммитом `a82be7b` после двух раундов нативного review — 5 находок `fixed-in 73645be`, 2 новые P2 вынесены в follow-up [#564](https://github.com/rbctmz/ai_trainer/issues/564)/[#565](https://github.com/rbctmz/ai_trainer/issues/565); см. `Change log` и `Artifacts and Notes`).
