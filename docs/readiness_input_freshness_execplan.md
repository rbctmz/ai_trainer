...
Ревизия документа: v3.10 (M1, M2 и срезы 3.1–3.3 приняты; срез 3.4 выполнен, M3 закрыт, см. `Change log`).
...
Остальные артефакты (M4–M6) появятся по мере реализации: независимое чтение `/api/today`, version-qualified ownership, выдержки из `ts_contract.json`.
