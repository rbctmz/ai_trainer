  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6">
      <div className="flex items-baseline justify-between">
        <h1 className="text-lg font-semibold text-ink">План vs факт</h1>
        <span className="text-xs text-ink-faint">
          {data?.ribbon_rule_version ?? ""}
        </span>
      </div>
      <p className="mt-1 text-sm text-ink-soft">
        Как исполняется активный план: лента последних недель по дням и
        недельные сводки. Подтверждение матчей — на странице{" "}
      ...
      {isLoading ? (
        ...
        <>
          ...
          <section className="mt-6">
            <h2 className="text-sm font-semibold text-ink">Лента по дням</h2>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              {Object.entries(STATUS_META).map(([key, meta]) => (
                <span
                  key={key}
                  className={`rounded-full px-2 py-0.5 ${meta.chip}`}
                >
                  {meta.label}
                </span>
              ))}
