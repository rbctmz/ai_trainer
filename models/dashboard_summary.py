...
def calculate_current_status(
    ...
    отображения остаётся прежним.

    ``acwr_as_of`` — якорь этого окна (обычно сегодняшний день): без него ряд
    заканчивается последней тренировкой, и дни отдыха после неё не достраиваются
    (находки ревью #595).
    """
